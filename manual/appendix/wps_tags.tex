...
\section{List Title (\fname{.sbs} only)}
  \begin{tagmap}
    ...
    \config{\%Lo} & Root main-menu title (RockPod): returns \config{s} when the list
      title matches \fname{LANG\_ROCKBOX\_TITLE} for the current language (same
      string as the top-level main menu), else empty. Use for locale-safe layout
      branching instead of comparing \config{\%Lt} to a hard-coded language.\\
